...
export PGHOST=keycloak-postgres-known-jaybird.postgres.database.azure.com
export PGUSER=psqladmin
export PGPORT=5432
export PGDATABASE=postgres
export PGPASSWORD="{your-password}" 
pg_dump -Fd -j 2 postgres -h keycloak-postgres-known-jaybird.postgres.database.azure.com -p 5432 -U psqladmin -N cron -f postgres_dump
pg_dump postgres -h keycloak-postgres-known-jaybird.postgres.database.azure.com -U psqladmin -p 5432 -N cron > postgres_dump.sql
createdb postgres_copy -h keycloak-postgres-known-jaybird.postgres.database.azure.com -p 5432 -U psqladmin
pg_restore -Fd -j 2 -d postgres_copy postgres_dump -h keycloak-postgres-known-jaybird.postgres.database.azure.com -p 5432 -U psqladmin 2> errors.log


docker run -it  -e PGPASSWORD=xRCvaM877*gePSeOuEdx@kOYdT postgres:16-alpine  psql  -h keycloak-postgres-known-jaybird.postgres.database.azure.com  -U psqladmin -p 5432 keycloak-db 
docker run -it  -e PGPASSWORD=my_password postgres:16-alpine pg_restore -Fd -j 2 -d postgres_copy postgres_dump -h keycloak-postgres-known-jaybird.postgres.database.azure.com -p 5432 -U psqladmin 2> errors.log


docker run -it  -e PGPASSWORD=my_password postgres:16-alpine  pg_dump  -h keycloak-postgres-known-jaybird.postgres.database.azure.com  -U psqladmin my_db > backup.sql
